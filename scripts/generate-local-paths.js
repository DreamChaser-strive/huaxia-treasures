/**
 * 生成本地图片路径映射表 src/data/artifactLocalPaths.js
 * 供 AR 页优先使用 /photosOrvideos/省/市/文物/1.jpg，失败则用原 img。
 * 运行：node scripts/generate-local-paths.js
 */
import fs from 'fs';
...
const cityToProvince = {
  ...
  '安阳市': '河南省', '洛阳市': '河南省', '许昌市': '河南省', '郑州市': '河南省', '开封市': '河南省', '荆州市': '湖北省', '武汉市': '湖北省',
...
const cityPinyin = {
  ...
  '济南市': 'jinan', '青岛市': 'qingdao', '曲阜市': 'qufu', '潍坊市': 'weifang', '安阳市': 'anyang', '洛阳市': 'luoyang', '许昌市': 'xuchang', '郑州市': 'zhengzhou', '开封市': 'kaifeng',
...
async function main() {
  ...
  for (const [cityKey, list] of Object.entries(artifactDatabase)) {
    ...
    for (const item of list) {
      ...
      const seg = `${provPy}/${cityPy}/${artPy}`;
      const key = (item.city || cityKey) + '\t' + name;
      map[key] = seg;
    }
  }
  const out = `/**
 * 文物本地图片路径映射（省/市/文物拼音），由 scripts/generate-local-paths.js 生成
 * AR 页优先使用 /photosOrvideos/{path}/1.jpg，加载失败则用原 img
 */
export const artifactLocalPaths = ${JSON.stringify(map, null, 0)};
`;
  const outPath = path.join(root, 'src', 'data', 'artifactLocalPaths.js');
  fs.writeFileSync(outPath, out, 'utf8');
  console.log('已生成 src/data/artifactLocalPaths.js，共', Object.keys(map).length, '条');
}
